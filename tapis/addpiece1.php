...
||empty($_POST['forme'] )||empty($_POST['origine'] )||empty($_POST['fabtype'] )||empty($_POST['dossier'] )||empty($_POST['color'] )||
empty($_POST['longueur'] )||empty($_POST['largeur'] )) {
    header('refresh:3;url=addpiece.php');
   echo" <center>
...
die () ;
  ...
  // write a qwery 
$sql=" insert into tapis (piece,titre,
mark,stock,description,point,prix,
reduit,composition,garantie,entretien,forme,origine,fabtype,dossier,color,longueur,largeur)
values('$piece','$titre',
'$mark','$stock','$description','$point','$prix',
'$reduit','$composition','$garantie','$entretien','$forme','$origine','$fabtype','$dossier','$color','$longueur','$largeur')" ;
...
// if retutne errer alor on a a syntax error in the query :
   ...
   if(!$result)  { header('refresh:2;url=addpiece.php');
        echo" <center>
   ...
   $sql=" select max(IdTapis)  from tapis " ;
   ...
   $result=mysqli_query($con,$sql) ;
      ...
      die () ;
       ...
       $idtapi=$row['max(IdTapis)'] ; // utulise pour image 
       header("location:form.php?IdTapis=$idtapi") ;
