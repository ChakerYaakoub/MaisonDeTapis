...
if(isset($_GET['submit'])){
    ...
    if(empty($_GET['piece'])||empty($_GET['mark'])||empty($_GET['origine'])
    ||empty($_GET['color'])||empty($_GET['minprix'])||empty($_GET['maxprix'])||empty($_GET['forme'])
    ||empty($_GET['minlongueur'])||empty($_GET['maxlongueur'])||empty($_GET['minlargeur'])||empty($_GET['maxlargeur'])){
        $_page=$_SERVER["HTTP_REFERER"] ;
       
        echo" <center>
         <b style='padding:30px; margin-top:30px; background-color:red; color:white; font-size:30px;' > Vous devez remplir toutes les informations </b>
     </center>" ;
     ...
     $maxlongueur=$_GET['maxlongueur'] ;
     $maxlargeur=$_GET['maxlargeur'] ;
     $minlongueur=$_GET['minlongueur'] ;
     $minlargeur=$_GET['minlargeur'] ;


     echo" <br><h > <center> <b style=' font-size: 35px; color: #3c3c3B; padding:20px; text-transform: uppercase;'> Outils de recherche </b></center></h></br>";
   ...
   $sql=" select *  from tapis where piece='$piece' and mark='$mark' and origine='$origine' and color='$color'
    and forme='$forme' and prix between 'minprix' and '$maxprix' 
    and longueur between 'minlongueur' and '$maxlongueur' 
    and largeur between 'minlargeur' and '$maxlargeur'  " ;
