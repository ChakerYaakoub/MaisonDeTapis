...
<body>
    ...
    <section>
        <?php
        if(empty( $_GET['UOrderDone']) || empty( $_GET['OrderIdUser'])){
            $_page=$_SERVER["HTTP_REFERER"] ; 
            header('refresh:3 ; url= '.$_page) ;
        echo" <center>
         <b style='padding:30px; margin-top:30px; background-color:red; color:white; font-size:30px;' > some informations is meesing </b>
     </center>" ;
     die () ; 

        }

        $UOrderDone=$_GET['UOrderDone'];
...
die () ; }
        ...
        $sql=" update OrderUser set UOrderDone='$UOrderDone'where OrderIdUser='$OrderIdUser' " ;
...
// if retutne errer alor on a a syntax error in the query :
   ...
   header("location:openOrdre.php?OrderIdUser=$OrderIdUser") ; 
